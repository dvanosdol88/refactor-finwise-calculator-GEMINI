...
// Force update
export function FlippableCard({ item, showBackLink = true }: FlippableCardProps) {
      ...
      >
        ...
        <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)]">
          ...
          <div className="flex h-full flex-col rounded-xl bg-gray-100 py-8 px-12 text-center">
            ...
            {showBackLink && item.link && (
              <a 
                href={item.link}
                target="_blank"
                rel="noopener noreferrer"
                className="mt-4 inline-flex items-center text-sm font-semibold text-primary hover:text-primary/90 hover:underline"
                onClick={(e) => e.stopPropagation()} // Prevent flipping back when clicking link
              >
                Learn Way More <ExternalLink className="ml-1 h-3 w-3" />
              </a>
            )}
          </div>
        </div>
