...
export default function DonutChart({
  ...
  // Calculation Logic (same as Calculator.tsx savings view)
  ...
  // 3. Lost to Fees (relative to no-fee growth)
  const lostToFees = grossFV - percentFeeFV;

  // 4. Kept Growth = Total Growth - Lost
  const totalGrowth = grossFV - portfolioValue;
  const keptGrowth = totalGrowth - lostToFees;

  const data = useMemo(() => [
    { name: 'Lost to Fees', value: lostToFees, color: 'hsl(var(--destructive))' }, // Red (Keep standard alert red for "Lost")
    { name: 'Your Profit', value: keptGrowth, color: 'hsl(var(--primary))' }, // Brand Green
  ], [lostToFees, keptGrowth]);

  // Force a clean re-render of the chart when core inputs change
  ...
  return (
    <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center relative bg-card/50 rounded-xl">
      <h4 className="text-lg font-semibold text-foreground mb-2">Where Your Growth Goes</h4>
      
      {/* Chart Container - Fixed Size for Stability */}
      <div className="relative w-[300px] h-[300px] flex items-center justify-center">
        <PieChart key={chartKey} width={300} height={300}>
          <Pie
            data={data}
            cx="50%"
            cy="50%"
      ...
      <div className="flex gap-4 mt-2 text-sm z-10">
        <div className="flex items-center gap-1">
          ...
          <span>Lost: <span className="font-bold text-destructive">{formatCurrency(lostToFees)}</span></span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded-full bg-primary" />
          <span>Kept: <span className="font-bold text-primary">{formatCurrency(keptGrowth)}</span></span>
        </div>
      </div>
    </div>
  );
