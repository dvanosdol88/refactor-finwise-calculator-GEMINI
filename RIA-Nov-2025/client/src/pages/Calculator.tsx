...
function CalculatorFinwiseView(props: CalculatorViewProps) {
  return (
    <MotionConfig reducedMotion="user">
      <div data-ui="finwise" className="min-h-screen bg-background text-foreground">
        ...
        <section className="relative overflow-hidden pt-24 md:pt-32 pb-12">
          ...
          <div className="mx-auto max-w-6xl px-5">
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-extrabold text-primary mb-6 tracking-tight">
              {props.viewMode === 'savings' ? (
                <>
                  What would <span className="relative inline-block text-primary">you<svg
                  ...
                  >
                    <defs>
                      <linearGradient id="brushGradientFW" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stopColor="#006044" />
                        <stop offset="50%" stopColor="#006044" />
                    ...
                    <Tooltip delayDuration={0}>
                      <TooltipTrigger asChild>
                        <span data-testid="text-total-savings" className="cursor-help inline-block decoration-dotted decoration-2 underline-offset-4 hover:text-primary">{formatCurrency(props.totalSavings)}</span>
                      </TooltipTrigger>
                      <TooltipContent
                        className="max-w-md rounded-[16px] shadow-xl p-5 z-50 bg-white border-0 ring-1 ring-black/5"
                        sideOffset={5}
                      >
                         <div className="text-sm space-y-3 font-sans">
                          <div>
                            <p className="font-bold text-gray-900 mb-1">Total Savings Calculation</p>
                            <p className="text-xs text-gray-500">This amount represents the difference in your total portfolio value after {props.years} years, comparing your current asset-based fee to our flat monthly fee.</p>
                          </div>
                          {/* Reusing the same tooltip content structure as legacy, simplified styles for preview */}
                          {/* (Ideally this tooltip content would be a shared component, but copying logic for safety as per instructions) */}
                          <div className="border-t border-gray-100 pt-3">
                             <div className="bg-primary/5 px-2 py-1.5 rounded font-mono text-sm font-semibold text-primary">
                              Difference: {formatCurrency(props.totalSavings)}
                            </div>
                          </div>
                        </div>
                      </TooltipContent>
                ...
                <div className="flex flex-col items-center">
                  <span>That covers <span className="text-primary">{props.spendingPowerYears.toFixed(1)} Years</span></span>
                  <span className="text-2xl sm:text-3xl mt-2 text-muted-foreground font-normal">of your annual spending!</span>
                </div>
              )}
            </h1>
            ...
            <div className="flex flex-col items-center gap-6">
              <div className="w-full max-w-sm">
               ...
               <button
                  ...
                  className="inline-flex items-center gap-2 text-sm font-semibold text-primary hover:bg-primary/5 px-4 py-2 rounded-full transition-colors"
                  onClick={() => {
                    if (navigator.share) {
                      navigator.share({
          ...
          <div className="grid lg:grid-cols-12 gap-6 items-start">
            {/* Left: Controls */}
            <FinwiseCard className="lg:col-span-4">
               <CalculatorControls
                portfolioValue={props.portfolioValue}
                setPortfolioValue={props.setPortfolioValue}
                annualFeePercent={props.annualFeePercent}
                setAnnualFeePercent={props.setAnnualFeePercent}
                years={props.years}
                setYears={props.setYears}
                portfolioGrowth={props.portfolioGrowth}
                setPortfolioGrowth={props.setPortfolioGrowth}
            ...
            <FinwiseCard
              ...
              className="lg:col-span-8 h-[500px] flex flex-col"
            >
              <ChartCarousel>
                <SavingsChart
                  data={props.chartData}
                  firstYearSavings={props.firstYearSavings}
      ...
      {/* FOOTER */}
      <footer className="py-12 text-center text-muted-foreground text-sm space-y-2">
